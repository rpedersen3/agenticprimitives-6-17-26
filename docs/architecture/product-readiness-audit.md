...
**Last refreshed:** 2026-06-10 — see [**2026-06-10 production-readiness audit**](../audits/2026-06-10-production-readiness-audit.md) (full post-remediation pass: all 29 packages re-audited by 5 independent reviews + all 42 contracts re-verified). Morning remediation wave closed CA-F1 / AN-1-ONCHAIN / ATT-1 / AGR-1 / ATT-3 / SIG-1 (verified in source; Base Sepolia redeployed) and made DEL-001 binding mandatory per-source. **Verdict: test/pilot-ready on testnet; NOT production-ready.** Remaining launch blockers: governance key rotation (CON-FACTORY-001), DEL-001 global default flip, NEW-MCP-1 (memory JTI NODE_ENV fall-open on Workers), CP-1/CP-2, PM-1/PM-2, third-party contract audit. 11 new findings registered in [`findings.yaml`](../audits/findings.yaml) (now 60 entries). Evidence: 774/774 forge + 27/27 package suites + freshness/typehash gates green.
**Prior refresh:** 2026-06-01 (R9 wave complete — 9 PRs landed: Foundry tune + Solhint + invariant suites for CustodyPolicy/DelegationManager/SmartAgentPaymaster; Halmos symbolic proofs of WebAuthn UV/UP + AgentAccount onlySelf closure; Echidna nightly + Medusa weekend stateful fuzzing; H-6 custodians cap closed + Slither/Aderyn triage; supply-chain CVE allowlist; spec 237 AEL + spec 238 v2 topology docs)
...
| **EXT-2** | **P1** | **EIP-712 typehash consistency (TS ↔ Solidity) lacks a CI gate.** | **CLOSED 2026-06-03.** The gate existed (`check:eip712-typehash-equality`); strengthened to read the LIVE `DelegationManager.sol` (8 assertions) and **promoted to PR-blocking** — now in `check:all` (run by `ci.yml` validate), not just `check:all-publish`. Registry-digest binding remains RW1-3 (a contract fix, not a missing gate). |
| **EXT-3** | **P2** | **WebAuthn `authenticatorData` length not validated** on the passkey verify path. | **CLOSED 2026-06-03 (already covered; verified no bypass).** On-chain verify bounds-checks: `WebAuthnLib.sol:91` (`authData.length < 37` → reject) + regression test `WebAuthnLib.t.sol:93 test_rejects_authData_too_short`. connect-auth is a CEREMONY HELPER (builds the assertion / parses registration), NOT a second verifier — and even it bounds-checks (`connect-auth/src/methods/passkey.ts parseAuthData:194` + attested-data guard). `WebAuthnLib` is the sole signature verifier. |
...
| **EXT-5** | **P2** | **`agent-relationships` privacy model.** | **CLOSED 2026-06-03 (doctrine, not a fix).** `agent-relationships` is structurally PUBLIC + experimental BY DESIGN; privacy is `related-agents` (private vault credentials). Made explicit in [ADR-0025](decisions/0025-related-agent-links-are-private.md) ("structurally public — keep it that way" + the consent-gated-publication product rule) + a HARD-STOP drift trigger in `packages/agent-relationships/CLAUDE.md`. Public org↔org assertion only after explicit bilateral consent; person↔org never a public edge. Not a gap — the correct deliberate split. |
