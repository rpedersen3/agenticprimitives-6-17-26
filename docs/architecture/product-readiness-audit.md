...
| --- | --- | --- | --- | --- |
| **N1** | Leaked deployer key controls live governance. | All factory + paymaster authority recoverable by anyone with the leaked key. | apps/contracts + ops | Deploy a clean deployer key OR transfer governance via existing `setBundlerSigner` / `setSessionIssuer` / `Ownable.transferOwnership` calls. Burn the old key. Withdraw old paymaster stake. |
| **C1** | Service-to-service authentication is not load-bearing. | A misrouted MCP endpoint relies only on bearer delegation tokens. | mcp-runtime, key-custody, apps | Implement canonical HMAC envelope verification for A2A-to-MCP, include nonce/replay cache, audience, route family, key ID, fail-closed. |
...
| **H2** | Policy classification is not enforced in MCP runtime. | Classified tools execute even if risk-tier policy would deny. | mcp-runtime, tool-policy | Run `evaluatePolicy()` inside wrapper; fail closed. |
...
| 2026-05-20 | Phase 4b refresh | Closed: H4 partial (passkey done), passkey arc, RPC-in-config leak. Added: N1 (leaked deployer key), N2 (`/account/derive-address` validation), N3 (paymaster monitoring), N4 (gas ceiling), N5 (live canary), N6 (orphaned contracts), N7 (passkey recovery). **Next:** top-5 quick-win pass targeting C4 + M3 + H1 + H3 + N2. |
