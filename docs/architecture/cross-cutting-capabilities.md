...
| --- | --- | --- | --- | --- |
| **Multi-sig + custody policy** | [`specs/207`](../../specs/207-smart-account-threshold-policy.md) (product surface, renamed from "threshold policy" in phase 6g.1) + [`specs/209`](../../specs/209-erc7579-module-taxonomy.md) (impl architecture) + [`specs/213`](../../specs/213-custody-layer-carve-out.md) (vocabulary firewall) | [`apps/demo-web-pro/docs/multi-sig/guide.md`](../../apps/demo-web-pro/docs/multi-sig/guide.md) | `agent-account`, `account-custody`, `delegation`, `tool-policy`, `audit`, `mcp-runtime` | Closed through phase 6g.3 — contract + SDK + runtime shipped end-to-end; ERC-7579 module decomposition landed (`CustodyPolicy` extracted from core, accounts under EIP-170); the custody / agency vocabulary firewall is implemented across contracts (`apps/contracts/src/{custody,agency}/`), SDK (`@agenticprimitives/account-custody`), and UI. 2 enforcers (`QuorumEnforcer`, `ApprovedHashRegistry`) deployed to Base Sepolia; redeploy of `CustodyPolicy` / `AgentAccount` impl / factory pending after phase 6g.* settles. |
...
| **DTK interop + caveat enforcer registry** | [`docs/architecture/dtk-alignment-audit.md`](dtk-alignment-audit.md) (audit) + [`specs/208`](../../specs/208-argument-level-caveats.md) (next enforcer) + [`docs/architecture/enforcer-registry/`](enforcer-registry/) (canonical registry) | TBD — likely `apps/demo-web-pro/docs/interop/guide.md` when first DTK-tooling consumer wires up | `delegation`, `tool-policy` | In-flight (phase 6b.1) — audit + registry + per-enforcer AUDIT.md shipped 2026-05-21. Enforcer contracts carry per-enforcer AUDIT.md at `apps/contracts/src/enforcers/<Name>.AUDIT.md`; T3+ delegations should carry argument-level caveats. `pnpm check:sentinel-enforcers` CI rail prevents new sentinel-only SDK exports. Remaining: ArgumentRuleEnforcer (spec 208 impl), interop fixture corpus (phase 7), permission-card renderer (phase 7). |
