...
| H-1 | high | Custody class branched on the cookie `via` (client-controlled, not bound to the verified credential). | **FIXED** (HW 2026-06-06) — `/fedcm/grant` now decides on the VERIFIED `principal` (`kind==='oidc' && role==='custody-grade'`) returned by `verifyHomeSession` from the signed AgentSession; the cookie `via` is never consulted for the decision. Closed by construction (demo-sso-next has no vitest harness for a unit test). |
| M-1 | med | Bridge nonce store is per-isolate in-memory (`getInMemoryNonceStore`) → cross-isolate replay within the 60s freshness window not caught (also affects `/custody/google/resolve`). Blast radius bounded: replay re-mints the SAME value-0 delegation. | **FIXED** (HW 2026-06-06) — provisioned a `BRIDGE_NONCES` KV namespace on demo-a2a; both bridge call sites use `nonceStoreFromKv` (cross-isolate single-use, 120s TTL) via `bridgeNonceStore(env)`, falling back to in-memory only when unbound (local dev). |
| M-2 | med | id_token replayable for the full 3600s TTL → `/fedcm/grant` re-issues an equivalent scoped delegation. | **FIXED** (HW 2026-06-06) — `/fedcm/grant` consumes the id_token's per-ceremony `nonce` one-time via KV (`fedcm-grant-nonce:<nonce>`, TTL = id_token TTL); replay → `409 nonce_replayed`. (Reused the unique nonce instead of adding a `jti`.) |
| L-1 | low | `assertionCorsHeaders` reflected the raw Origin with `Allow-Credentials:true` before the registered-origin check → response bodies readable by any reflected origin. | **FIXED** (HW 2026-06-06) — `/fedcm/grant` + preflight set ACAO ONLY when `isAllowedRelyingOrigin(origin)`; unknown origin → no ACAO. |
| I-1 | info | `verifyHomeSession` aud-retry (`fedcm.ts`) is a `try-strong/catch-weaker` shape (ADR-0013 smell). Not exploitable — signature still verified, `isOwnConnectOrigin` gates issuer, same-subject check binds. | **OPEN-HW** — left intact (retry accommodates cookie tokens minted with varying aud; removing risks dropping valid sessions). |
...
Must-fix-before-Base-Sepolia: **M-3 (done)**. Hardening wave (2026-06-06): **H-1, M-2, L-1 FIXED**. Remaining tracked: **I-1**, **I-2** (info-level cleanups). M-1 fixed (BRIDGE_NONCES KV).
