...
|---|---|---|---|---|---|---|---|
| SEC-001 | 🔴 P0 | `server/oidc/grant.ts` | No server-side enrollment-grant binding; any signed delegation → id_token for any client. | No | 🔴 OPEN | H6 OIDC closure | ADR-0019, spec 230 §4.2 |
| SEC-002 | 🔴 P0 | `server/token.ts` | Silent-re-auth id_token `aud` not bound to delegation's caveats; cross-client replay. | No | 🔴 OPEN | H6 OIDC closure | ADR-0019, SEC-003 |
| SEC-003 | 🔴 P0 | `apps/demo-{jp,org}/src/connect-client.ts` + `apps/demo-a2a/wrangler.toml` | Shared default delegate SA + paymaster signer collision. | Yes (partial) | 🔴 OPEN | H6 OIDC closure | ARCH-004, CT-1 |
...
| SEC-005 | 🟠 P1 | `src/components/onboarding/useEnrollReq.ts` + `src/whitelabel/config.ts` | `ALLOWED_RELYING_ORIGINS` hardcoded + out of sync with `relyingApps[].redirect_uris`. | No | 🔴 OPEN | H6 OIDC closure | ARCH-002, ADR-0013 |
| SEC-006 | 🟠 P1 | `server/_lib/origin.ts` | `iss` derived from attacker-controlled `Host` header; no issuer allowlist at broker or RP. | No | 🔴 OPEN | H6 OIDC closure | spec 232 §5 |
...
| SEC-009 | 🟡 P2 | `server/connect/enroll.ts` + `src/lib/kv-indexer.ts` | No rate limit; `kv.put` overwrites prior facet (silent agent bridging on credKey collision). | No | 🔴 OPEN | Operational lane | — |
| SEC-010 | 🟠 P1 | `server/oidc/google/callback.ts` + `server/_lib/server-broker.ts` | Single `A2A_CUSTODY_BRIDGE_SECRET` controls every Google × KMS resolve. | No | 🔴 OPEN | H7 Custody hygiene | spec 235 §G-1, KH-5 |
| SEC-011 | 🟡 P2 | `server/_lib/verify-delegation.ts` + `server/connect/{passkey,siwe}.ts` | `isDeployed` polls up to 12–15s in the request path; trivial DoS. | No | 🔴 OPEN | Operational lane | — |
| SEC-012 | 🟡 P2 | `apps/demo-{jp,sso-next}/src/csrf.ts` | Module-cached CSRF token; non-HttpOnly cookie; no rotation handling. | No | 🔴 OPEN | Operational lane | — |
| SEC-013 | 🟡 P2 | `app/(portal)/profile/page.tsx` | Profile handoff page lacks CSRF binding to active home session. | No | 🔴 OPEN | H8 Handoff hardening | SEC-004 |
| SEC-014 | 🟡 P2 | `src/lib/pii.ts` | Custody gate sound but data is hard-coded stub; doctrine to re-assert namespace on every handler. | Yes (data) | 🔴 OPEN | R/N follow-up | ARCH-015 |
| SEC-015 | 🟡 P2 | `src/lib/passkey.ts` + `connect-client.ts:228` | Discoverable assertion's rawId not checked client-side against expected credentialIdDigest. | No | 🔴 OPEN | Operational lane | — |
| SEC-016 | 🟡 P2 | `src/lib/sso-cookie.ts` | Parent-domain `ap_sso` cookie is JS-readable; sign-out doesn't always clear. | Yes (TODO) | 🔴 OPEN | H7 Custody hygiene | — |
| SEC-017 | 🟡 P2 | `server/connect/link-{lookup,request}.ts` | `/connect/link/lookup` returns full pubkey + label for any code; no rate limit. | No | 🔴 OPEN | Operational lane | — |
| SEC-018 | 🟢 P3 | `apps/demo-jp/src/connect-client.ts:443-447` | Silent re-auth pins `iss` to user-typed name; no issuer allowlist at RP. | No | 🔴 OPEN | H6 OIDC closure | SEC-006 |
| SEC-019 | 🟢 P3 | `apps/demo-jp/src/App.tsx:83-114` `restoreSession` | Session restore reads localStorage + decodes JWT without re-verifying signature. | No | 🔴 OPEN | H6 OIDC closure | — |
| SEC-020 | 🟢 P3 / ℹ️ | `apps/demo-jp/src/lib/domain.ts:21-46` | `personalAuthOrigin` doesn't hard-validate label character set; bounded today, regression-prone. | No | 🔴 OPEN | Operational lane | — |
| SEC-021 | ℹ️ | `apps/demo-jp/functions/jwks.ts` + `functions/connect/*` | demo-jp Pages Functions broker is dead code (SPA never calls it); attack surface for no benefit. | No | 🔴 OPEN | Operational lane (delete) | — |
| SEC-022 | ℹ️ | `apps/demo-jp/src/lib/matches.ts:230-280` | Dual-persona's `fac-self-<addr>` invites cross-record collision; production needs separate SAs. | Yes (demo) | 🔴 OPEN | R/N follow-up | ARCH-021 |
