...
| **NM-11** | `identity-auth` JWT claim `agentName?: string` accepted, signed, NOT resolved by identity-auth. | open — NS Phase 2 | spec 215 § 7 Phase 2 row + ADR-0006 § "Refused: name registration as passkey-enrolment side-effect" | — | — |
...
| `identity-auth` | 0 | 0 | 1 | passkey UV policy default ("preferred" — should be "required" for high-assurance) |
| `agent-account` (contracts) | 0 | 0 | 1 | SB-2 (ECDSA `s` normalization on non-GCP paths) |
| `types` | 0 | 0 | 0 | — (NameContext + AgentType added 2026-05-23; consumed via injection by audit / tool-policy / delegation / mcp-runtime / identity-auth in NS Phase 2) |
| `agent-naming` | 0 | 0 | 5 | NA-1..NA-5 — Phase 1 invariants closed (NM-1..NM-5 + NM-14 + NM-15); NM-6..NM-12 open pending NS Phase 2/3/4 |
| `agent-identity` | 0 | 0 | 0 | architecture locked (spec 217 + ADR-0007 + ADR-0008); Phase 1 scaffold pending → NM-16 |
...
| 2026-05-23 | NS lockdown (ADR-0007 + ADR-0008 + specs 216/217) | Added NM-14 (CAIP-10 grammar validation, closed via code change in same turn — 7 new tests in records.test.ts). Added NM-15 (no UAID derivation — invariant by absence per ADR-0008). Added NM-16 (agent-identity package architecture, Phase 1 pending). Added NM-17 (agent-relationships package architecture, Phase 1 pending). Per-package rollup gains `agent-identity` + `agent-relationships` rows (0/0/0 — architecture locked, no findings yet). |
