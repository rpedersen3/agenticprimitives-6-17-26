...
| **Scope** | `packages/*` source + `apps/contracts/{src,test,script}` ONLY. Demo apps and any specs that describe app integration (spec 234 white-label, spec 236 JP adoption, spec 232/229/230§6) are **explicitly out of scope**. |
...
| **Status** | Open — this is the living tracker for the production-library audit. |
...
|---|---|---|---|---|
| 1 | **PKG-DELEGATION-001** | 🔴 Critical | `packages/delegation/src/evaluator.ts:42,53,65` | Off-chain caveat evaluator returns `allowed: true` for `Value`, `AllowedTargets`, `AllowedMethods` when context is missing ("enforced on-chain"). Any consumer using the evaluator off-chain (MCP gate, A2A pre-check, any non-redeem path) silently permits the call when the on-chain enforcement never fires. |
| 2 | **PKG-KEY-CUSTODY-001** | 🔴 Critical | `packages/key-custody/src/factories.ts:89-96` + `src/derive-subject.ts:90-105` | `buildToolExecutorBackend(toolId, opts)` returns the master signer (per-tool HKDF deferred); `deriveSubjectPrivateKeyHex` is a raw-master entry point exported from `src/index.ts` outside the public manifest. Any well-meaning consumer wiring "agent calls tools" ends up with the master key in the tool execution path. |
| 3 | **ARCH-006 / ARCH-038 / PKG-key-custody-001 (arch)** | 🔴 Critical (doctrine) | `packages/key-custody/{src/index.ts,capability.manifest.json}` | **`pnpm check:all` is RED.** `check:public-exports` fails: `key-custody` exports 5 symbols (`deriveSubjectSigner`, `deriveSubjectPrivateKeyHex`, `subjectCanonicalMessage`, `SubjectId`, `DeriveSubjectOpts`) not in the manifest's `publicExports`. Spec 100 §4 makes the manifest the boundary. **This is the single most embarrassing find** — a senior reviewer will see redacted-vs-actual mismatch on the most security-sensitive package and assume the doctrine is performative. |
| 4 | **CON-NAMING-001** | 🔴 High | `apps/contracts/src/naming/AgentNameRegistry.sol:135-163` + `script/Deploy.s.sol:170-175` | `initializeRoot` is fully permissionless. Any mempool observer can front-run the deployer's first `initializeRoot("agent", deployer, …)` and own `.agent` forever. Same for any future TLD. The deploy script doesn't bundle deploy+init atomically. |
| 5 | **PKG-CONNECT-AUTH-001 + PKG-CONNECT-AUTH-002** | 🔴 High | `packages/connect-auth/src/verify-signature.ts:108-118,130-147` + `src/salt.ts:22-30` | (a) `verifyUserSignature{,View}` conflate RPC failure with "signature invalid" via `catch { return false }` — caller cannot distinguish chain-down from forged-sig. (b) `deriveSaltFromEmail(email, rotation)` makes the smart-account address publicly enumerable from an email address: anyone can pre-compute a target's SA address and link addresses across services. |
| 6 | **PKG-MCP-RUNTIME-001 + CON-WEBAUTHN-001 + CON-Libraries-001/-002** | 🔴 High | `packages/mcp-runtime/src/jti-stores.ts:53-108`; `apps/contracts/src/libraries/{WebAuthnLib.sol,P256Verifier.sol}` | (a) JTI store adapters silently run `CREATE TABLE IF NOT EXISTS` with runtime credentials; least-privilege deploys silently disable replay protection. (b) On-chain WebAuthn verifier checks the challenge bytes but NOT the RP ID hash, NOT `clientDataJSON.origin`, NOT UP/UV flags — cross-origin signing oracle accepted. (c) `WebAuthnLib` 16% line coverage, `P256Verifier` 0% direct line coverage — security-critical surfaces under-tested. |
| 7 | **XPKG-001 + XPKG-003** | 🔴 High | All `packages/*/package.json` + `specs/100` + `docs/architecture/*.md` | (a) Every package's `license` is `UNLICENSED` despite the repo-root MIT LICENSE that landed in commit `89ee037`. npm and supply-chain scanners read the package field. A third party who runs `npm-license-checker` against an extracted SDK gets the legal answer "no license" and walks away. (b) Spec 100 §3 — the load-bearing identity-doctrine table — still names `identity-auth`, `agent-identity`, `custody` (renamed half a year ago to `connect-auth`, `agent-profile`, `account-custody`). Doctrine docs and the code disagree. |
...
| ID | Severity | Component | One-line | Source | Status |
|---|---|---|---|---|---|
| **PKG-CONNECT-AUTH-001** | 🔴 High | `src/verify-signature.ts:108-147` `verifyUserSignature{,View}` | `catch { return false }` conflates "signature invalid" with "RPC timed out" / "validator contract not deployed". Caller cannot distinguish chain-down from forged-sig. Direct ADR-0013 violation. Caller cannot mitigate without monkey-patching `args.client.readContract`. **Remediation pending — open task for hardening wave:** return `{ ok: true } \| { ok: false, reason: 'invalid' \| 'rpc' }`. | SEC | 🔴 OPEN |
| **PKG-CONNECT-AUTH-002 / PKG-connect-auth-001** | 🔴 High | `src/salt.ts:22-30` `deriveSaltFromEmail` | Canonical Smart Agent address is a public deterministic function of the user's email. Address-from-email enumeration; cross-product correlation; linkability across services. Contradicts ADR-0010 (SA address IS the canonical identifier — no public function from side-channel identifier to that address). EXT-030 re-cast at the package API surface. | SEC + ARCH | 🔴 OPEN |
| PKG-CONNECT-AUTH-003 | 🟡 Medium | `src/sessions.ts:108-151` `verifySession` | Does not pin `aud`/`iss`/`sub`; caller MUST do additional checking. Multiple tokens issued for different audiences with the same HS256 key are mutually replayable. Also: `loadKeys()` called inside `verifySession` — caller cannot inject the secret outside of `process.env`. EXT-028 re-cast. | SEC | 🔴 OPEN |
| PKG-CONNECT-AUTH-004 | 🟡 Medium | `src/csrf.ts` | Reads `origin` from the token itself; caller responsible for binding actual request `Origin` header. Tokens not bound to path/method/body/session — a token usable on `POST /transfer` is also usable on `POST /grant-admin`. EXT-029 re-cast. | SEC | 🔴 OPEN |
...
| **PKG-CONNECT-001 (sec)** | 🟠 High | `src/token.ts:121-156` `verifyAgentSession` | `expectedAud` and `expectedIss` are OPTIONAL. If a caller forgets `expectedAud`, the audience is not verified. Token issued for audience-A accepted at audience-B. Contract should be `expectedAud: string` (required). `verifyIdToken` (line 211-217) DOES require these — the AgentSession variant is the regression. | SEC | 🔴 OPEN |
...
| **PKG-KEY-CUSTODY-002 / PKG-key-custody-001** | 🔴 Critical | `src/index.ts` + `capability.manifest.json:publicExports` | `pnpm check:public-exports` FAILS. `src/derive-subject.ts` exports `deriveSubjectSigner`, `deriveSubjectPrivateKeyHex`, `subjectCanonicalMessage`, `SubjectId`, `DeriveSubjectOpts` (spec 235 Google × KMS work). NONE in manifest. **`deriveSubjectPrivateKeyHex` leaks the raw master + per-subject derivation** — returns per-subject private key as hex string in memory. ARCH-006 / ARCH-038 STILL failing. **Single most embarrassing find.** | SEC + ARCH | 🔴 OPEN |
| PKG-KEY-CUSTODY-003 | 🟠 High | `src/providers/local.ts:197-244` `LocalSecp256k1Signer.signA2AAction` | Uses `secp256k1.sign(input.digest, this.priv)` without enforcing low-s normalization. Noble defaults to low-s (canonical TODAY), but invariant undocumented — future noble bump or callsite passing `lowS: false` is silent signature-malleability regression. SB-2 marked `partial` in evidence checklist; GCP signer does `normalizeLowS(s)` explicitly, local does not. | SEC | 🔴 OPEN |
| PKG-KEY-CUSTODY-004 | 🟠 High | `src/providers/gcp.ts:296-330` `findRecoveryByte` | On failure does `console.error` with `digest`, `r`, `s`, `knownPubKey`. In worker logging pipelines these are searchable; the digest is the userOp hash. Not secret but fingerprintable. P3 info-disclosure. | SEC | 🔴 OPEN |
...
| PKG-mcp-runtime-001 | 🟠 High | `src/with-delegation.ts:357-358` `verifyCrossDelegationForResource` | Pair to PKG-delegation-001 (arch). Public export, stub. Experimental subpath OR throw OR delete. | ARCH | 🔴 OPEN |
...
| ID | Severity | Component | One-line | Source | Status |
|---|---|---|---|---|---|
| **PKG-agent-profile-001** | 🟠 High | `src/errors.ts:4,14,22,29` + `src/client.ts:50,52,55,95` + docs | Rename from `agent-identity` → `agent-profile` left source error messages stamped `[agent-identity]`. Users of `@agenticprimitives/agent-profile` get errors with old package name. 62 stale refs across CLAUDE.md, AUDIT.md, README.md, spec.md, docs/, src/. **Sweep `[agent-identity]` → `[agent-profile]`.** | ARCH | 🔴 OPEN |
...
| CON-AgentAccount-003 | 🟡 Medium | Storage layout 50-slot gap, ERC-7201 namespaces for passkeys + modules | Layout correct; storage doc good. **Add automated storage-layout snapshot test (`forge inspect AgentAccount storageLayout`) committed so any future ordering bug fails CI.** | 🔴 OPEN |
...
| CON-SmartAgentPaymaster-002 | 🟢 Low | `setDevMode(true)` + production preflight | OP-1 says `check:production-deploy` fails on dev-mode. Verify preflight reads `_dev` via public getter; contract exposes `dev()` only as private state. Verify `isDev()` or equivalent exists. | 🔴 OPEN |
...
| ID | Severity | Component | One-line | Status |
|---|---|---|---|---|
| **XPKG-001** | 🟠 High | All `packages/*/package.json` | Every package's `license` is `UNLICENSED` despite repo-root MIT LICENSE. npm and supply-chain scanners read the package field. EXT-011 closed only the repo root. **Closure:** set `"license": "MIT"` in all 16 manifests + add `LICENSE` to each package's `files` array. | 🔴 OPEN |
| **XPKG-002** | 🟠 High | `packages/delegation` + `packages/mcp-runtime` | `verifyCrossDelegation` stub in two packages. Public symbols whose existence implies capability the runtime can't do. ADR-0013 violation. Decide once: experimental subpath OR delete. | 🔴 OPEN |
...
| **XCON-003** | 🟡 Medium | No storage-layout snapshot tests | None of `AgentAccount`, `CustodyPolicy`, `DelegationManager` commit a `forge inspect storageLayout` snapshot. UUPS upgrade safety on `AgentAccount` is code-review-only invariant today; layout drift bug ships silently. | 🔴 OPEN |
...
**Contracts**:
- `apps/contracts/src/{AgentAccount,AgentAccountFactory,SmartAgentPaymaster,ApprovedHashRegistry,UniversalSignatureValidator}.sol`
- `apps/contracts/src/agency/DelegationManager.sol`
- `apps/contracts/src/custody/CustodyPolicy.sol`
- `apps/contracts/src/enforcers/{AllowedTargets,AllowedMethods,QuorumEnforcer,Timestamp,Value,CaveatEnforcerBase}.sol`
- `apps/contracts/src/libraries/{WebAuthnLib,P256Verifier,SignatureSlotRecovery,MultiSendCallOnly}.sol`
- `apps/contracts/src/naming/{AgentNameRegistry,PermissionlessSubregistry,AgentNameUniversalResolver}.sol`
- `apps/contracts/script/Deploy.s.sol`
- `apps/contracts/deployments-base-sepolia.json`
