...
| 4 | **CON-NAMING-001** | 🔴 High | `packages/contracts/src/naming/AgentNameRegistry.sol:135-163` + `script/Deploy.s.sol:170-175` | `initializeRoot` is fully permissionless. Any mempool observer can front-run the deployer's first `initializeRoot("agent", deployer, …)` and own `.agent` forever. Same for any future TLD. The deploy script doesn't bundle deploy+init atomically. |
