...
| **Status** | Living tracker — opened 2026-05-30. Body tally (2026-05-31, post-R3.1): **37 CLOSED / 2 ACCEPTED / 155 OPEN** of 193 findings. Only **1 Critical** finding remains OPEN (`PKG-agent-relationships-001` — privacy fork labeling). **R1** closed `CROSS-STACK-001` (high) + `CON-AgentAccount-003` + `XCON-003` (medium). **R3.1 (audit-row reconciliation, 2026-05-31)** flipped 8 stale rows where the code work had completed in H7 but the audit row hadn't been updated: `PKG-KEY-CUSTODY-001` (H7-B.1), `PKG-KEY-CUSTODY-002` (H7-B.1), `PKG-DELEGATION-001` (H7-B strict-mode default), `PKG-DELEGATION-002` (H7-B default-closed failmode), `PKG-CONNECT-001-sec` (H7-B.4 expectedAud required), `CON-WEBAUTHN-001` (H7-C.1 RP-ID pin), `CON-NAMING-001` (H7-C.4 atomic init), `CON-P256-001` (H7-C.2 Daimo removed). The Executive summary §1 below is the original opening snapshot; Top-7 was regenerated 2026-05-31 (post-R3.1) to reflect current confirmed-OPEN highest-severity findings. |
...
|---|---|---|---|---|
| 1 | **PKG-agent-relationships-001** | 🔴 Critical-when-published | `packages/agent-relationships/*` | Public on-chain edge model is **structurally incompatible** with any confidentiality requirement a third-party adopter brings (the Privacy Fork, EXT-019). Package must label `"stability": "experimental"` loudly, README top-of-page "do not use for confidential edges" callout, and document the threat model. Spec 239 (private relationship store) is the alternative. *(R3.2 wave assigned.)* |
| 2 | **CON-AgentAccount-001** | 🟡 High | `packages/contracts/src/AgentAccount.sol` + forge coverage | R3.4 lifted to **82.19% lines / 80.00% statements / 56.34% branches / 96.55% functions** (was 55.94%/52.99%/42.25%/62.07%). Target ≥90% lines / ≥80% branches before external audit. Remaining gap: WebAuthn signature flows + ERC-7579 module install/invoke cycle + `addPasskey`/`removePasskey` recovery + `_authorizeUpgrade` via real UUPS upgrade. R3.5 wave assigned. Floor in `check-forge-coverage.ts` raised to 82/55 — no regression possible. |
| 3 | **PKG-AUDIT-001** | 🟠 High | `packages/audit/src/index.ts:182-200` `composeSinks` | Fail-soft swallows per-sink errors with only `console.error`; **security-critical events (delegation grant, custody change, signing operations) need durable-before-commit semantics**. Split into `composeFailSoftSinks` (telemetry) vs `composeFailHardSinks` (security-critical; throws on first failure). Spec 214 OP-2 unmet by current API shape. (Promoted from rank 4 after R3.3 closed CON-DelegationManager-001 — coverage 95.77% lines / 88.24% branches.) |
| 4 | **PKG-DELEGATION-003** | 🟠 High | `packages/delegation/src/token.ts:131-230` `mintDelegationToken` | Defaults `ttlSeconds` to 10 min but production ceiling is 60 min. A consumer copying-and-extending from a demo can set TTL up to 1 hour without `acceptElevatedRisk`. 1-hour token with `usageLimit: 100` is real production blast radius. Recommend hard ceiling of 15 minutes and `usageLimit: 10` without `acceptElevatedRisk`. |
| 5 | **PKG-DELEGATION-004 / PKG-KEY-CUSTODY-003** | 🟠 High | `packages/delegation/src/token.ts` + `packages/key-custody/src/providers/local.ts:197-244` | No low-s normalization on the EIP-191 sign+recover path or on `LocalSecp256k1Signer.signA2AAction`. Noble defaults to low-s today; invariant undocumented. Future noble bump or `lowS: false` callsite is a silent signature-malleability regression. JTI tracking uses `claims.jti` value (good) so reply-tracking holds, but auditors will flag this. GCP signer already does explicit `normalizeLowS(s)` — local signer should mirror. |
| 6 | **PKG-MCP-RUNTIME-001 / 002** | 🟠 High | `packages/mcp-runtime/src/{jti-stores.ts:53-108, with-delegation.ts:286-307,357-358}` | (a) JTI store adapters silently run `CREATE TABLE IF NOT EXISTS` with runtime credentials; least-privilege deploys silently disable replay protection. Should expose `migrate()` and reject if schema missing in hot path. (b) `withDelegation`'s `requires-consent` branch recomputes `evaluateThresholdPolicy(opts.classification).requiresAcceptedOnChain` even though `requireAcceptedOnChain` already threaded into verify — fragile encoding can desync on future refactor. (c) `verifyCrossDelegationForResource` is a not-implemented stub still on the public surface (paired with PKG-delegation-001-arch). |
| 7 | **CON-DEPLOY-001 / XCON-001** | 🟠 High | `script/Deploy.s.sol` + governance aggregation | Single deployer-EOA controls all governance / roles / TLDs at the end of `forge script Deploy`. **Single-key compromise = total system takeover.** Hand-off ceremony (Safe + Timelock) is documented but is a post-deploy *manual* operation, not a constraint enforced by the deploy script. Cross-cutting with `CON-FACTORY-001`, `CON-NAMING-001` (post-R3.1 closure), `CON-PAYMASTER-002`. |
...
|---|---|---|---|---|
| **CON-AgentAccount-001** | 🟠 High | Forge coverage | **55% lines / 52% statements / 38% branches.** Below "production-library" bar for ERC-4337 + UUPS-upgradeable + ERC-7579 module-host + WebAuthn-supporting account. Critical uncovered: `executeFromModule` callback edges, `_authorizeUpgrade`, hook iteration limits (`MAX_HOOKS`), `addPasskey`/`removePasskey` lifecycle from `RecoverAccount`. **Target ≥90% lines / ≥80% branches before any external audit.** Add invariant/fuzz tests for: passkey set membership consistency, custodian count ≡ externalCustodianCount + piaCount, module installed flag ↔ installedList[type] membership. | 🟡 IN PROGRESS (R3.4 / 2026-05-31) — lifted to **82.19% lines / 80.00% statements / 56.34% branches / 96.55% functions** via new `test/AgentAccountCoverage.t.sol` (45 tests) exercising: every public getter (`entryPoint`, `bundlerSigner`, `sessionIssuer`, `version`, `accountId`, `supportsModule`, `supportsExecutionMode`, `getInstalledModules`, `upgradeTimelock`, `pendingUpgrade`, `delegationManager`, `factory`, `isAgenticPrimitivesAgentAccount`), the `execute(address,uint256,bytes)` + `executeBatch(Call[])` paths with onlySelf gate / EntryPoint caller / value forwarding / target-revert propagation / empty array, the upgrade lifecycle entry points (`setUpgradeTimelock`, `cancelPendingUpgrade`, `executePendingUpgrade`, `upgradeToWithAuthorization`), session-delegation acceptance + query, custodian round-trip add/remove + onlySelf gating, `supportsInterface` ERC-165 sentinels, `setDelegationManager` happy + reject path, `passkeyIdentity` determinism, and the full `executeFromBundler` ERC-4337 outer-gate (FactoryNotSet revert / NotBundler bundler-sig revert / happy path / InvalidInnerSignature revert) which covers `_verifySignerEcdsa` + the inner `_validateSig` path. `scripts/check-forge-coverage.ts` floor raised from 55%/40% to 82%/55%. **Remaining gap to ≥90% / ≥80%:** WebAuthn signature generation flows (`_validateSignature` via PackedUserOp), ERC-7579 module install/uninstall lifecycle with mock validator/executor/hook modules, `addPasskey`/`removePasskey` recovery flows, `_authorizeUpgrade` exercised via real UUPS upgrade. Folded into a follow-up wave; debt floor ratchets prevent regression. |
