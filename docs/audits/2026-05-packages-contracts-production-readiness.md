...
| 2 | **PKG-AUDIT-001** | 🟠 High | `packages/audit/src/index.ts:182-200` `composeSinks` | Fail-soft swallows per-sink errors with only `console.error`; **security-critical events (delegation grant, custody change, signing operations) need durable-before-commit semantics**. Split into `composeFailSoftSinks` (telemetry) vs `composeFailHardSinks` (security-critical; throws on first failure). Spec 214 OP-2 unmet by current API shape. (Promoted to rank 2 after R3.5 closed CON-AgentAccount-001 — coverage 91.25% lines / 84.51% branches / 100% functions.) |
| 3 | **PKG-DELEGATION-003** | 🟠 High | `packages/delegation/src/token.ts:131-230` `mintDelegationToken` | Defaults `ttlSeconds` to 10 min but production ceiling is 60 min. A consumer copying-and-extending from a demo can set TTL up to 1 hour without `acceptElevatedRisk`. 1-hour token with `usageLimit: 100` is real production blast radius. Recommend hard ceiling of 15 minutes and `usageLimit: 10` without `acceptElevatedRisk`. |
| 4 | **PKG-DELEGATION-004 / PKG-KEY-CUSTODY-003** | 🟠 High | `packages/delegation/src/token.ts` + `packages/key-custody/src/providers/local.ts:197-244` | No low-s normalization on the EIP-191 sign+recover path or on `LocalSecp256k1Signer.signA2AAction`. Noble defaults to low-s today; invariant undocumented. Future noble bump or `lowS: false` callsite is a silent signature-malleability regression. JTI tracking uses `claims.jti` value (good) so reply-tracking holds, but auditors will flag this. GCP signer already does explicit `normalizeLowS(s)` — local signer should mirror. |
| 5 | **PKG-MCP-RUNTIME-001 / 002** | 🟠 High | `packages/mcp-runtime/src/{jti-stores.ts:53-108, with-delegation.ts:286-307,357-358}` | (a) JTI store adapters silently run `CREATE TABLE IF NOT EXISTS` with runtime credentials; least-privilege deploys silently disable replay protection. Should expose `migrate()` and reject if schema missing in hot path. (b) `withDelegation`'s `requires-consent` branch recomputes `evaluateThresholdPolicy(opts.classification).requiresAcceptedOnChain` even though `requireAcceptedOnChain` already threaded into verify — fragile encoding can desync on future refactor. (c) `verifyCrossDelegationForResource` is a not-implemented stub still on the public surface (paired with PKG-delegation-001-arch). |
| 6 | **CON-DEPLOY-001 / XCON-001** | 🟠 High | `script/Deploy.s.sol` + governance aggregation | Single deployer-EOA controls all governance / roles / TLDs at the end of `forge script Deploy`. **Single-key compromise = total system takeover.** Hand-off ceremony (Safe + Timelock) is documented but is a post-deploy *manual* operation, not a constraint enforced by the deploy script. Cross-cutting with `CON-FACTORY-001`, `CON-NAMING-001` (post-R3.1 closure), `CON-PAYMASTER-002`. |
| 7 | **PKG-CONNECT-AUTH-002** | 🟠 High | `packages/connect-auth/src/salt.ts:22-30` | `deriveSaltFromEmail(email, rotation)` makes the smart-account address **publicly enumerable from an email address**: anyone who knows a person's email can pre-compute their SA address + link addresses across services. Privacy regression. Mitigations: rotation requires opt-in; documented as "anti-pattern, do not use for confidentiality-requiring users"; or replaced by a per-(iss,sub) HKDF that's NOT computable without the master KMS. |
