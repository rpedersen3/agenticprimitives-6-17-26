...
| **CON-ENFORCER-PAUSE-001** | ℹ️ (R6 follow-up to CON-AgentAccount-005) | Enforcer pause-coverage architectural invariant | R6.1 recon § 2.4 raised an open question: are enforcer `beforeHook` / `afterHook` reachable outside paused `DelegationManager.redeemDelegation`? If yes, they need their own pause checks. | SEC | 🟢 CLOSED (R6.7 / 2026-05-31) — **Audit conclusion: enforcer pause checks are unnecessary.** Architectural invariant verified: (1) All 5 production enforcer hooks are declared `external pure` or `external view`; Solidity prevents state mutation at the compiler level. (2) Every enforcer has zero storage variables (manually verified — the only `address prev` in `QuorumEnforcer` is a LOCAL variable inside a for-loop, not storage). (3) `DelegationManager.redeemDelegation` checks `governance.isPaused()` at lines 149-154 BEFORE the for-loops that dispatch beforeHook (line 287) / afterHook (line 308); when paused, the DM reverts `SystemPaused` BEFORE any enforcer is touched. (4) A caller invoking an enforcer hook directly during a pause sees the same revert/no-revert behaviour as any other time — there is nothing to drain, no state to corrupt. Each of the 5 enforcers (`ValueEnforcer`, `AllowedTargetsEnforcer`, `AllowedMethodsEnforcer`, `TimestampEnforcer`, `QuorumEnforcer`) carries a `R6.7 — Stateless validator` docstring referencing the recon doc + this regression test. 4 new R6.7 tests in `test/EnforcerPauseInvariantR67.t.sol` lock the invariant: (a) `DM_paused_revertsBeforeReachingEnforcer` uses a `SideEffectfulEnforcer` mock with a callCount; counter stays at 0 after a paused redeem call confirms the DM gate fires first; (b) `DM_unpaused_doesReachEnforcer` sanity-checks the inverse; (c) `directEnforcerCall_isStatelessForValueEnforcer` proves repeated direct calls are pure-functional; (d) `allProductionEnforcersAreStorageless` deploys each enforcer as a checklist marker — if a future change adds storage to any of them, the architectural invariant breaks and this test should be replaced with per-enforcer pause checks. **No code change to enforcers beyond docstring annotations.** 4/4 R6.7 tests + 549/549 full contracts suite green. |
...
| **CON-CustodyPolicy-005** | 🟠 High (R6 follow-up to CON-AgentAccount-005) | `CustodyPolicy.sol` system-pause coverage | R6.1 recon § 2.3 identified that `CustodyPolicy` had **ZERO pause checks across the schedule/apply/cancel surface**. When governance pauses the system, an attacker holding quorum sigs can still schedule + apply custody changes. Pause is supposed to be the emergency switch — currently it doesn't switch off the custody machinery. | SEC | 🟢 CLOSED (R6.6 / 2026-05-31) — New `whenAccountNotPaused(address account)` modifier + `_systemPausedFor(account)` helper. Helper chains 3 staticcalls: `account.factory() → factory.governance() → governance.isPaused()`. Any non-conforming hop (EOA account / EOA factory / EOA governance / missing fn / no code) returns `false` for legacy / test compatibility (mirrors R6.5 + `GovernanceManaged._pausedSafe()`). New `error SystemPaused()`. Modifier applied to **2 mutating entrypoints** that grant/transfer authority: `scheduleCustodyChange`, `applyCustodyChange`. **2 recovery primitives deliberately left UNGUARDED** — operator must always be able to de-escalate during an incident: `cancelScheduledChange` (defensive cancellation), `onUninstall` (removing the custody module). `onInstall` is gated upstream by R6.5's paused `AgentAccount.installModule` and the factory's paused `createAgentAccount`, so no per-call modifier needed. 3 minimal mock contracts (`MockAccount`, `MockFactory`, `MockGovernancePausable`) exercise the staticcall chain. 8 new R6.6 tests: 2 paused-reverts, 2 unpaused-doesn't-revert-with-SystemPaused, 2 recovery-still-works-when-paused, 2 legacy-EOA-{account,governance}-never-pauses. 8/8 R6.6 tests + 552/553 full suite (only failure: pre-existing R5.9 env-bleed). |
...
| **Forge coverage in CI with thresholds** | High | `forge coverage` not in `ci.yml`. XCON-001 at 59%; threshold gate would have caught WebAuthnLib regression to 16%. 🟢 **PARTIAL CLOSED (R6.9 / 2026-05-31)** — New `pnpm coverage:contracts` aggregator (`scripts/coverage-contracts.ts`) parses LCOV output to surface coverage for ALL 28 src/ contracts, **including the security-critical ones that the summary table silently skips** (AgentAccount, AgentAccountFactory, SmartAgentPaymaster, UniversalSignatureValidator, DelegationManager, CustodyPolicy, the 5 enforcers). Emits per-contract JSON + markdown summary. Current baseline: 11 security-critical contracts → 82.1% lines / 57.4% branches / 90.1% functions. One contract below the 70% security-critical line floor: SmartAgentPaymaster at 50.9% (clear R6.10 target). Gate is INFORMATIONAL today; flips to ENFORCING after R6.10. Existing `check:forge-coverage` ratchet preserved unchanged. |
