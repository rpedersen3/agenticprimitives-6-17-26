...
| **Phase 3** | Port `AgentNameRegistry`, `AgentNameAttributeResolver`, `AgentNameUniversalResolver` to `apps/contracts/src/naming/`. Simplifications: no `AgentRelationship` dependency (parent-pointer encoded directly in registry node), no OpenZeppelin `AccessControl` / `TimelockController` (the owner Smart Agent's CustodyPolicy IS the timelock + RBAC; registry calls `IERC1271.isValidSignature(owner, hash, sig)` for authorization). Forge tests. Deploy + persist addresses. | after Phase 2 |
