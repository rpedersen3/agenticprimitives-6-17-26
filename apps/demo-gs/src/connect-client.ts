// OIDC relying-app client for demo-gs (spec 230 / ADR-0019).
//
// demo-gs is a RELYING APP that does NOT run its own broker — every sign-in goes
...
// finding — the unused server-side functions in `apps/demo-gs/functions/connect/*`
...
const CLIENT_ID = 'demo-gs';
const redirectUri = (): string => window.location.origin + '/';

// demo-gs's fixed relying-site delegate identity — the backend account JP grants are
// scoped TO. The person's secure home (Impact) issues the scoped grant to THIS address;
// the backend presents it for delegated reads. Configurable via VITE_DEMO_GS_DELEGATE.
...
const DEMO_GS_DELEGATE: Address =
  ((import.meta.env?.VITE_DEMO_GS_DELEGATE as string | undefined) ??
 ...
 *  person→org mapping — just the org metadata + the scoped delegation demo-gs received. */
...
/** Decode the id_token payload (the home already ran + verified the ceremony). For full
 *  alg-pinned / iss-allowlisted trust use {@link verifyIdToken}. */
export function decodeIdToken(idToken: string): IdTokenClaims {
  return decodeJwtSegment<IdTokenClaims>(idToken.split('.')[1] ?? '');
}

/** The person SA address carried by an id_token (the `sub` is a CAIP-10 canonical agent id). */
export function personAddressFromIdToken(idToken: string): Address {
  const c = decodeIdToken(idToken);
  const raw = c.canonical_agent_id ?? c.sub;
  return (raw.includes(':') ? raw.split(':').pop()! : raw) as Address;
}

// ── Public surface ─────────────────────────────────────────────────────

/** Build the OIDC `/authorize` request to the person's secure home (template=site-login).
 *  demo-gs creates NOTHING locally — no passkey, no account, ZERO prompts on this origin.
 *  The home runs the ceremony and redirects back with `?code=…&state=…`. This is the KC
 *  individual login + the first step of the GCO signatory connect. */
 ...
 *  AT their home (same custody as their person agent — demo-gs is never a custodian);
 *  the home mints a scoped org→demo-gs delegation and returns it with the org identity.
