...
/**
 * Read `isTrustee(account, signer)` on the CustodyPolicy. Trustees are
 * authorized for T6 RecoverAccount; for T1-T5 admin actions the
 * relevant check is `isCustodian` on the target account instead. Returns
 * false if the policy isn't installed (read reverts).
 */
export async function readIsTrustee(args: {
  custodyPolicy: Address;
  account: Address;
  signer: Address;
}): Promise<boolean> {
...
export async function readIsCustodian(args: {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      // Treat transient RPC failures (rate limit, network blip, replica
      // lag returning a revert) as "not yet true" and keep polling. The
      // verify step shouldn't crash on a flaky read; if the chain truly
      // doesn't reflect the recovery, the loop times out and reports
      // `false` to the caller.
      last = false;
    }
    if (i < attempts - 1) await new Promise((r) => setTimeout(r, intervalMs));
