// Popup-based central-auth ceremony (spec 257 Phase 1 Wave 2 — ported VERBATIM from demo-gs's
// audit-hardened `src/lib/central-auth.ts`; the cross-app-duplication pattern the repo already uses).
// demo-jp opens the central auth in a FIRST-PARTY popup (WebAuthn create/get work there; no iframe
// storage partitioning) and receives the result over a strictly origin-validated postMessage channel.
// Redirect is the fallback when the popup is blocked or on mobile (ADR-0013 explicit fallback).
//
// Security (audit F3/F5): we resolve ONLY on a message from the exact popup window we opened, at the
// exact central-auth origin, whose echoed `state` matches what we generated. The OP (spec 230) returns
// a single OIDC authorization CODE over the popup channel; the relying site exchanges it at /token. No
// tokens/delegations travel in the message or URL.
//
// The broker side (demo-sso-next `useEnrollReq.ts`) already speaks this exact protocol — it detects
// `?mode=popup` + `window.opener` and posts `{type:'AC_SUCCESS', state, code}` to the exact relying
// origin, then `window.close()`. This file is the relying-side counterpart only.
 ...
 *  `ac_relay` marker. Only demo-jp windows (this origin) can read it; the listener still validates
 *  `state` so a relayed code can't cross-bind to a different in-flight connect. */
export const RELAY_CHANNEL = 'demo-jp-connect-relay';
