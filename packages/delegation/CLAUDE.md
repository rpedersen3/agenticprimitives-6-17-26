...
- **Relying-site (cross-origin) authority** — see [`docs/relying-site-delegation.md`](docs/relying-site-delegation.md) (the trade-off + shape) + [ADR-0019](../../docs/architecture/decisions/0019-relying-site-authority-is-a-scoped-delegation.md) + [spec 229](../../specs/229-personal-central-auth.md). A relying site is a **scoped delegate** of the person SA (NOT a custodian): the person's ROOT credential signs a caveated `Delegation` (`issueDelegation` + `hashDelegation`) to the site key; runtime auth = a live, unrevoked, in-window delegation; revoke via `revokeDelegationByOwner`. This package backs spec-229 P6.
