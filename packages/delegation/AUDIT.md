...
| --------------- | -------- | -------------------------------------------------------------------------------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------- |
| **DEL-001**     | Critical | Token verifier didn't bind the presenting session key to the delegated authority — observe-and-re-mint = delegator impersonation. | **CLOSED 2026-06-10 · production-enforced** | `sessionDelegateBindingError` requires `sessionDelegation.delegator === delegation.delegator` + `delegate === sessionKeyAddress`, validated via the `UniversalSignatureValidator`. Binding is now **fail-closed by default** ([ADR-0036](../../docs/architecture/decisions/0036-session-delegate-binding-is-fail-closed-by-default.md)): enforced unless the caller passes the greppable `allowUnboundSessionToken: true` opt-out. The prior `requireSessionDelegateBinding`/`strictSessionBinding` flags are removed. Tests: `test/unit/verify-universal-validator.test.ts`, `test/unit/remint-attack.test.ts`. spec 270 v4. |
