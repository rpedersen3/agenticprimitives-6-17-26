# @agenticprimitives/geo-features — AUDIT

Off-chain geo-CLAIM SDK (spec 251). Transport-agnostic; no on-chain claim writes.
...
- **`GEO_KIND` is lockstep with `GeoFeatureRegistry.KIND_*`** (cross-stack test reads the live `.sol`).
...
## Trust boundary
A claim's authority is the issuer signature over `geoEndorsementDigest` (verified off-chain via ERC-1271)
+ the pinned `(featureId, version)` existing on the registry (`geoFeatureExists`). This SDK derives those
