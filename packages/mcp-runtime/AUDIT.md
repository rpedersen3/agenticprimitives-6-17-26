...
| --- | --- | --- | --- | --- |
| **C1** (system) | P0 | Service-to-service MAC not load-bearing. | Open | This package will own the verification step; depends on `key-custody.buildMacProvider`. |
| **C3** (system) | P0 | No audit-event emission from `withDelegation`. | Open | Should emit on accept + reject. |
| **H2** (system) | P1 | `tool-policy.evaluatePolicy()` not called. | Open | Marked as future in `with-delegation.ts:6`. |
...
- **Unit:** 2 files, 8 tests as of 2026-05-20:
