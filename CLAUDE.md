...
- **No deployment-domain code in packages.** `packages/*` are domain- and deployment-AGNOSTIC. Concrete hostnames (`impact-agent.me`/`impact-agent.io`), the demo `.agent` subregistry (`demo.agent`), hosting providers (`*.pages.dev`, `*.workers.dev`, `vercel`), subdomain/host parsing, and any "where is this deployed" knowledge live ONLY at the **app** layer (`apps/*`) — each app owns its domain config and passes values in (config/env/args). The `.agent` TLD itself is the naming protocol (owned by `agent-naming`) and is allowed; the `demo.` subregistry + concrete origins are not. Enforced by `pnpm check:no-domain-in-packages` ([ADR-0021](docs/architecture/decisions/0021-no-deployment-domain-code-in-packages.md)). Each app SHOULD centralize its domain literals in one module (e.g. `src/lib/domain.ts`), not scatter them.
